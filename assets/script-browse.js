...
const getGameByNameDebounce = async (event) => {
  ...
  try {
    ...
    //This is just a check to see if any day was discovered the in following forEach loop. 
    //If something was found, the count goes up. If nothing was found, 
    //the count returns to 0 and the lone if statement is fired
    count = 0;
    ...
    data.forEach((data) => {
      if (data.title.includes(userInputChoiceValue)) {
        ...
        gameInformation += `
        <div class="card-body">
        <div class="game-display" style="width: 18rem;>
                          <a href='${data.game_url}'><img src='${data.thumbnail}' alt="image of the game searched"/>
                          </a>
                          <a href='${data.game_url}'><p>${data.title}</p>
                          </a>
...
const fetchWithCheckBoxAndSearchBar = async (event) => {
  ...
  //Looping through each checkbox, looking for the checkboxes that are checked. 
  //I store those checkboxes that are checked to the new array "checkedCheckboxes"
...
//* author: Olu - changing name for CSS purposes
document
  // .getElementById("btn-submit-search")
  //? could this work ?
  .getElementById("search-bar-for-game")
